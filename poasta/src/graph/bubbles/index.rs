    ...
    N: GraphNodeId,
{
        ...
        G: GraphWithNodeOrdering<NodeType = N> + GraphWithNodeLengths<NodeType = N>,
    {
        ...
        for n in finder.inv_rev_postorder().iter().rev() {
            ...
            let min_dist = graph
                .successors(*n)
                .map(|succ| dist_to_end[graph.node_rank(succ)].0 + node_length)
                .min()
                .unwrap_or(0);

            let max_dist = graph
                .successors(*n)
                .map(|succ| dist_to_end[graph.node_rank(succ)].1 + node_length)
                .max()
                .unwrap_or(0);
            ...
            let (slice1, slice2) = node_bubble_map.split_at_mut(n_rank);
            ...
            graph
                ...
                .flat_map(|succ| {
                    ...
                    if succ_rank < n_rank {
                        slice1[succ_rank].iter()
                    } else {
                        slice3[succ_rank - n_rank - 1].iter()
                    }
                })
    ...
    #[inline]
    pub fn is_entrance(&self, node_rank: usize) -> bool {
        self.bubble_entrance[node_rank].is_entrance()
    }

    #[inline]
    pub fn is_exit(&self, node_rank: usize) -> bool {
        self.bubble_exit[node_rank].is_exit()
    }
    ...
    #[inline]
    pub fn node_is_part_of_bubble(&self, node_rank: usize) -> bool {
        !self.node_bubble_map[node_rank].is_empty()
    }
    ...
    #[inline]
    pub fn get_min_dist_to_end(&self, node_rank: usize) -> usize {
        self.dist_to_end[node_rank].0
    }

    #[inline]
    pub fn get_max_dist_to_end(&self, node_rank: usize) -> usize {
        self.dist_to_end[node_rank].1
    }
