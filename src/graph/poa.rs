...
pub(crate) mod graph_impl {
    ...
    mod tests {
        ...
        fn test_aligned_interval_basic() {
            let aln_ival =
                AlignedInterval::<usize>::new(10, 5, POANodeIndex::<usize>::new(0usize), 20);
        ...
        fn test_aligned_interval_other_split() {
            let aln_ival = AlignedInterval::<usize>::new(10, 5, POANodeIndex::<usize>::new(1), 20);
        ...
        fn test_intersect() {
            ...
            let other_aln_ival =
                AlignedInterval::<usize>::new(12, 6, POANodeIndex::<usize>::new(1), 22);
...
pub use graph_impl::{AlignedInterval, POANodeIndex};
    ...
    Ix: IndexType,
{
    ...
    pub fn path_for_sequence(&self, seq_id: usize) -> Vec<POANodeIndex<Ix>> {
        ...
        loop {
            ...
            let next_node = self.graph.neighbors(*last_node).find(|v| {
    ...
    pub fn path_sequence(
        &self,
        path: impl IntoIterator<Item = POANodeIndex<Ix>>,
    ) -> Result<Vec<u8>, GraphError<Ix>> {
        let mut seq = Vec::new();
    ...
    pub fn get_node_aligned_intervals(&self, node: POANodeIndex<Ix>) -> &[AlignedInterval<Ix>] {
        &self.graph[node].aligned_intervals
    }
    ...
    fn add_node_with_data(&mut self, node_data: POANodeData<Ix>) -> POANodeIndex<Ix> {
        ...
        if self.node_count() == 3 {
            ...
            self.graph.remove_edge(
                self.graph
                    .find_edge(self.start_node, self.end_node)
                    .unwrap(),
            );
            self.graph.add_edge(
                self.start_node,
                new_ix,
                POAEdgeData::new_with_seq_id(self.sequences.len()),
            );
            self.graph.add_edge(
                new_ix,
                self.end_node,
    ...
    pub fn add_aligned_sequence(
        ...
        if self.is_empty() {
            ...
            {
                let new_node = self
                    .add_node_with_weights(sequence.as_ref().to_vec(), weights.as_ref().to_vec());
                self.sequences
                    .push(Sequence(sequence_name.to_string(), new_node));
            }
        ...
        while let Some(block) = blocks.next_block(self, seq, &nodes_split) {
            match block.block_type {
                AlignmentBlockType::Match {
                    ...
                    pred = Some(self.process_matches(
                        w,
                        qry_range,
                        &node_ivals,
                        pred,
                ...
                AlignmentBlockType::MismatchNew {
                    qry_range,
                    node_ivals,
                } => {
                ...
                AlignmentBlockType::MismatchPresent {
                    qry_range,
                    other_node,
                } => {
                    pred = Some(self.process_mismatches_present(
                        w,
                        qry_range,
                        other_node,
                        pred,
        ...
        self.sequences
            .push(Sequence(sequence_name.to_string(), start_node.unwrap()));
        self.post_process(&nodes_split)?;
    ...
    fn process_matches(
        ...
        if first_node_pos.pos() > 0 {
            ...
            let (left_node, right_node) =
                self.split_node_at(first_node_pos.node(), first_node_pos.pos());
            nodes_split.add_split(
                first_node_pos.node(),
                first_node_pos.pos(),
                left_node,
                right_node,
        ...
        if let Some(p) = pred {
            ...
            self.graph
                .add_edge(p, new.node(), POAEdgeData::new_with_seq_id(new_seq_id));
        }

        let (last_node_pos, ival_length) = node_ivals.last().unwrap();
        let last_poa_node_pos = nodes_split.to_new_node_pos(POANodePos(
            last_node_pos.node(),
            last_node_pos.pos() + *ival_length,
        ));
        let last_node_seq_len = self
            .graph
            .node_weight(last_poa_node_pos.node())
            .unwrap()
            .sequence
            .len();
        ...
        if last_poa_node_pos.pos() < last_node_seq_len {
            ...
            let (left_node, right_node) =
                self.split_node_at(last_poa_node_pos.node(), last_poa_node_pos.pos());
            nodes_split.add_split(
                last_poa_node_pos.node(),
                last_poa_node_pos.pos(),
                left_node,
                right_node,
            );
    ...
    fn process_mismatches_new(
        ...
        for (node_pos, length) in node_ivals {
            let aligned_interval =
                AlignedInterval::new(curr_node_pos, *length, node_pos.node(), node_pos.pos());
            debug!(
                "Add aligned interval new node to -> {:?}, {}",
                node_pos.node(),
                node_pos.pos()
            );
        ...
        debug!(
            "Added node with sequence length {} (new node ix: {:?})",
            qry_range.len(),
            new_qry_node_ix
        );
        ...
        for (node_pos, length) in node_ivals {
            ...
            let aln_ival_to_qry =
                AlignedInterval::new(node_pos.pos(), *length, new_qry_node_ix, curr_node_pos);

            debug!(
                "Add aligned interval aligned node {:?}, {} to -> query {:?}, {}",
                node_pos.node(),
                node_pos.pos(),
                new_qry_node_ix,
                curr_node_pos
            ...
            for aln_ival_to_other in &node_data.aligned_intervals {
                ...
                if let Some((ival_other_to_query, ival_query_to_other)) =
                    aln_ival_to_other.transitive_intersect(&aln_ival_to_qry)
                {
                    // Node was already aligned to 'other', and the aligned interval intersects with the
                    // aligned interval to the new query.
                    debug!(
                        "Transitive alignment other {:?} -> qry {:?}",
                        aln_ival_to_other.other(),
                        new_qry_node_ix
                    );
                    ...
                    debug!(
                        "Transitive alignment qry {:?} -> other {:?}",
                        new_qry_node_ix,
                        aln_ival_to_other.other()
                    );
        ...
        for aln_ival in additional_aln_ivals_for_qry {
            debug!(
                "Add additional aligned interval query {:?}, {} to -> other {:?}, {}",
                new_qry_node_ix,
                aln_ival.node_start(),
                aln_ival.other(),
                aln_ival.other_start()
            );
        ...
        if let Some(from_node) = pred {
            self.graph.add_edge(
                from_node,
                new_qry_node_ix,
                POAEdgeData::new_with_seq_id(new_seq_id),
            );
        }
    ...
    fn process_mismatches_present(
        ...
        if let Some(p) = pred {
            if other_node.pos() == 0 {
                ...
                let (left_node, right_node) =
                    self.split_node_at(other_node.node(), other_node.pos());
        ...
        let end_node_seq_len = self
            .graph
            .node_weight_mut(end_node.node())
            .unwrap()
            .sequence
            .len();
    ...
    fn add_msa_sequence(
        ...
        for (qpos, npos, curr_state) in &aln {
            match (prev_state, *curr_state) {
                ...
                (AlignmentClassification::Match, AlignmentClassification::Match) => {
                    ...
                    if prev_match_node.is_some()
                        && prev_match_node != npos.map(|p| nodes_split.to_new_node_pos(p).node())
                    {
        ...
        self.sequences
            .push(Sequence(seq_name.to_string(), start_node.unwrap()));
    ...
    fn process_msa_matches(
        ...
        if node_pos_start.pos() > 0 {
            let (left_node, right_node) =
                self.split_node_at(node_pos_start.node(), node_pos_start.pos());
            nodes_split.add_split(
                node_pos_start.node(),
                node_pos_start.pos(),
                left_node,
        ...
        let last_node_seq_len = self
            .graph
            .node_weight(last_node_pos.node())
            .unwrap()
            .sequence
            .len();
        if last_node_pos.pos() < last_node_seq_len - 1 {
            let (left_node, right_node) =
                self.split_node_at(last_node_pos.node(), last_node_pos.pos() + 1);
            nodes_split.add_split(
                last_node_pos.node(),
                last_node_pos.pos() + 1,
                left_node,
    ...
    fn process_msa_mismatches(
        ...
        let mut aln_ivals: FxHashMap<POANodeIndex<Ix>, AlignedInterval<Ix>> = FxHashMap::default();
        for (i, (qpos, _, _)) in aln.iter().enumerate() {
            for node_pos in msa_nodes_cover.get_nodes_for_col(*qpos) {
                aln_ivals
                    ...
                    .or_insert_with(|| AlignedInterval::new(i, 1, node_pos.node(), node_pos.pos()));
            }
        }
        ...
        if let Some(p) = pred {
            self.graph
                .add_edge(p, new_node_ix, POAEdgeData::new_with_seq_id(new_seq_id));
        }
    ...
    fn process_msa_insertion(
        ...
        if let Some(p) = pred {
            self.graph
                .add_edge(p, new_node, POAEdgeData::new_with_seq_id(new_seq_id));
        }
    ...
    fn post_process(&mut self, nodes_split: &SplitTracker<Ix>) -> Result<(), GraphError<Ix>> {
        ...
        for e in self.graph.edges(self.start_node) {
            ...
            if in_degree_target > 1 {
                debug!(
                    "Removing edge {:?} -> {:?}, in_degree_target: {}",
                    e.source(),
                    e.target(),
                    in_degree_target
                );
        ...
        for n in self.graph.node_indices() {
            ...
            if self.graph.edges_directed(n, Incoming).count() == 0 {
                let mut all_outgoing_ids: Vec<_> = self
                    .graph
            ...
            if self.graph.edges_directed(n, Outgoing).count() == 0 {
                let mut all_incoming_ids: Vec<_> = self
                    .graph
        ...
        for e in to_add {
            debug!("Adding edge {:?}", e);
            self.graph
                .add_edge(e.0, e.1, POAEdgeData::new_with_seq_ids(e.2));
        }
    ...
    fn split_node_at(
        ...
        debug!(
            "Split node {:?} at position {}. New nodes: {:?} (len: {}), {:?} (len: {})",
        ...
        let mut intervals_left: Vec<_> = node_data
            .aligned_intervals
            .iter()
        ...
        let mut intervals_right: Vec<_> = node_data
            .aligned_intervals
            .iter()
            .filter(|ival| ival.node_start() < pos && ival.node_end() > pos)
        ...
        self.graph
            .node_weight_mut(left_node)
            .unwrap()
            .aligned_intervals = intervals_left;
        self.graph
            .node_weight_mut(right_node)
            .unwrap()
            .aligned_intervals = intervals_right;
        ...
        let seq_ids: Vec<_> =
            itertools::kmerge(self.graph.edges_directed(node, Incoming).map(|e| {
                e.weight()
                    .sequence_ids
                    .iter()
                    .copied()
        ...
        self.graph.add_edge(
            left_node,
            right_node,
            POAEdgeData::new_with_seq_ids(seq_ids),
        );
    ...
    Ix: IndexType,
{
    pub(crate) fn add_split(
        ...
        node: POANodeIndex<Ix>,
        split_pos: usize,
        left: POANodeIndex<Ix>,
        right: POANodeIndex<Ix>,
    ) {
...
mod tests {
    ...
    use crate::{
        aligner::utils::AlignedPair,
        graph::{
            alignment::POANodePos,
            io::dot::graph_to_dot,
            poa::{graph_impl::AlignedInterval, SplitTracker},
        },
    };
    ...
    fn test_split_node() {
        ...
        // Do the split
        let (lnode, rnode) = { graph.split_node_at(orig_node, 10) };
        ...
        assert_eq!(
            &orig_node2_data.aligned_intervals,
            &[
    ...
    fn test_add_alignment_mismatches() {
        ...
        graph
            .add_aligned_sequence("seq2", seq2, &weights2, Some(&alignment))
            .unwrap();

        let seq_truth: Vec<&[u8]> = vec![b"#", b"GTCTGCTAT", b"GGG", b"ACT", b"GCGTACGTCGT", b"$"];
        ...
        graph
            .add_aligned_sequence("seq3", seq3, &weights3, Some(&alignment))
            .unwrap();
    ...
    fn test_insertion() {
        ...
        let seq_truth: Vec<&[u8]> = vec![b"#", b"GTCTGCTAT", b"TTAAT", b"ACTGCGTACGTCGT", b"$"];
    ...
    fn test_deletion() {
        ...
        let seq_truth: Vec<&[u8]> = vec![b"#", b"GTCTGCTAT", b"ACT", b"GCGTACGTCGT", b"$"];
        ...
        for (rank, n) in graph.toposorted.iter().enumerate() {
            ...
            println!(
                ...
                rank,
                n,
                std::str::from_utf8(&node_data.sequence).unwrap()
            );
            println!("{} {:?}   w: {:?}", rank, n, node_data.weights);
            println!("{} {:?} aln: {:?}", rank, n, node_data.aligned_intervals);
    ...
    fn test_from_msa() {
        ...
        assert_eq!(
            graph
                .node_data(node_data.aligned_intervals[0].other())
                .sequence,
            b"C"
        );
    }
